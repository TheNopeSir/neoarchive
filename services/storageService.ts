...
const CACHE_VERSION = '2.5.1-SystemStatsFix'; 
...
export const registerUser = async (username: string, password: string, tagline: string, email: string, telegram?: string): Promise<UserProfile> => {
    const userProfile: UserProfile = {
        username, email, tagline, telegram,
        avatarUrl: getUserAvatar(username),
        joinedDate: new Date().toLocaleString('ru-RU'),
        following: [], achievements: ['HELLO_WORLD'], isAdmin: false,
        status: 'ONLINE'
    };
...
export const loginUser = async (email: string, password: string): Promise<UserProfile> => {
    const user = cache.users.find(u => u.email === email || u.username === email); // Simple mock check for now or rely on sync
    if (user) return user;
    // Fallback if not found in cache (should be handled by real auth in full version)

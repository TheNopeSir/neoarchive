...
const CommunityHub: React.FC<CommunityHubProps> = ({ theme, users, exhibits, onExhibitClick, onUserClick, onBack }) => {
    ...
    const [duelStage, setDuelStage] = useState<'LOBBY' | 'SELECT' | 'FIGHT'>('LOBBY');
    ...
    // Top Users: (Posts*5) + (Likes*1) + (Followers*2)
                ...
                {tab === 'TRENDS' && (
                    <div className="space-y-8 animate-in slide-in-from-right-4">
                        ...
                        <div className={`p-4 rounded-2xl border ${isWinamp ? 'bg-[#292929] border-[#505050]' : 'bg-gradient-to-r from-yellow-900/10 to-transparent border-yellow-500/20'}`}>
                            <h3 className={`text-xs mb-4 flex items-center gap-2 uppercase tracking-widest ${isWinamp ? 'text-wa-gold font-winamp' : 'text-yellow-500 font-pixel'}`}><Trophy size={14}/> Top Active Collectors</h3>
                        ...
                        {/* Hot Items */}
                        <div>
                            <h3 className={`text-xs mb-4 flex items-center gap-2 uppercase tracking-widest ${isWinamp ? 'text-wa-gold font-winamp' : 'text-red-400 font-pixel'}`}><Flame size={14}/> Hot Right Now</h3>
                ...
                {tab === 'TRADE' && (
                    <div className="animate-in slide-in-from-right-4">
                        <div className={`mb-6 p-4 border rounded-xl text-center ${isWinamp ? 'border-[#505050] bg-[#292929]' : 'border-blue-500/30 bg-blue-500/5'}`}>
                            <h3 className="text-sm mb-1 font-bold">MARKETPLACE</h3>
                            <p className="text-[10px] opacity-60">Только проверенные лоты. Безопасные сделки через гаранта (в разработке).</p>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                tradeExhibits.map(item => (
                                    <div key={item.id} className="relative">
                                        ...
                                        <div className={`absolute top-2 left-2 px-2 py-1 text-[10px] font-bold rounded shadow-lg ${isWinamp ? 'bg-black text-[#00ff00] border border-[#00ff00]' : 'bg-black/80 text-white backdrop-blur-md'}`}>
                                            {item.tradeStatus === 'FOR_SALE' ? `${item.price || '?'} ${item.currency || 'RUB'}` : 'TRADE'}
                                        </div>
                                    </div>
                                ))
                            )}
                        ...
                        {!showCreateGuild ? (
                            <div className="space-y-4">
                                <button 
                                    ...
                                    className={`w-full py-4 border-2 border-dashed rounded-xl flex items-center justify-center gap-2 hover:bg-white/5 transition-all ${isWinamp ? 'border-[#505050] text-wa-green' : 'border-white/20'}`}
                                >
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {activeGuilds.map(guild => (
                                        <div key={guild.id} className={`p-6 border rounded-xl flex items-center justify-between cursor-pointer hover:bg-white/5 transition-all ${isWinamp ? 'bg-[#292929] border-[#505050]' : 'bg-white/5 border-white/10'}`}>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] opacity-60 font-mono">{guild.members.length} Members • Leader: {guild.leader}</p>
                                                </div>
                                            </div>
                                            <button 
                                                onClick={() => joinGuild(guild.id, 'CurrentUser')} 
                                                className={`px-3 py-1 text-[9px] border rounded uppercase opacity-50 hover:opacity-100 ${isWinamp ? 'border-wa-green text-wa-green' : ''}`}
                                            >
                                                Вступить
                                            </button>
                                        </div>
                            ...
                            <div className="p-6 border rounded-xl">
                                ...
                                />
                                <input 
                                    className="w-full bg-black/20 border p-2 mb-4" 
                                    placeholder="Описание"
                                    value={newGuildDesc}
                                    onChange={(e) => setNewGuildDesc(e.target.value)}
                                />
                                <div className="flex gap-2">
                                    <button onClick={handleCreateGuild} className="bg-green-500 text-black px-4 py-2 rounded">Создать</button>
                                    <button onClick={() => setShowCreateGuild(false)} className="border border-white/20 px-4 py-2 rounded hover:bg-white/10">Отмена</button>
                                </div>
                            </div>
                        )}
                    </div>

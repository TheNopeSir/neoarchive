...
const SearchView: React.FC<SearchViewProps> = ({ 
    ...
    return (
        <div className="max-w-4xl mx-auto animate-in fade-in pb-32">
            <div className="flex items-center gap-4 mb-6 sticky top-20 z-30 pt-4 bg-transparent backdrop-blur-md pb-4 border-b border-white/10">
                <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                    <ArrowLeft size={20} />
                </button>
                <div className={`flex-1 flex items-center px-4 py-3 rounded-2xl border ${theme === 'dark' ? 'bg-black/40 border-white/20' : 'bg-white border-black/10'}`}>
                    ...
                    <div className="space-y-4">
                        ...
                        filteredUsers.map(u => (
                            <div 
                                ...
                                className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer hover:bg-white/5 ${theme === 'dark' ? 'border-white/10 bg-dark-surface' : 'border-black/10 bg-white'}`}
                            >
                                <div className="w-12 h-12 rounded-full overflow-hidden border border-white/20">
                                    <img src={u.avatarUrl || getUserAvatar(u.username)} className="w-full h-full object-cover" />
                                </div>
                                <div>
                                    <div className="font-bold font-pixel text-sm">@{u.username}</div>
                                    <div className="text-[10px] font-mono opacity-60">{u.tagline}</div>
                                </div>
                            </div>
                        ))}
                    </div>

...
const DirectChat: React.FC<DirectChatProps> = ({ 
    ...
    return (
        <div className="max-w-2xl mx-auto flex flex-col h-[calc(100vh-140px)] animate-in fade-in">
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-white/5 rounded-t-3xl">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="font-pixel text-xs font-bold">@{partnerUsername}</div>
            ...
            <form onSubmit={handleSend} className="p-4 bg-white/5 border-t border-white/10 rounded-b-3xl">
                <div className="flex gap-2">
                    <input 
                        value={input}
                        onChange={e => setInput(e.target.value)}
                        placeholder="ВВЕСТИ СООБЩЕНИЕ..." 
                        className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-3 font-mono text-sm focus:outline-none focus:border-green-500 transition-all"
                    />
